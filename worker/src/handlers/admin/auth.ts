...
function b64UrlEncode(data: string | ArrayBuffer | Uint8Array): string {
...
export async function login(c: Ctx) {
  ...
  const user = await queryOne<{ id: string; email: string; password_hash: string; name: string; role: string }>(
    c.env,
